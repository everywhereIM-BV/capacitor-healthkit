...
public class CapacitorHealthkitPlugin: CAPPlugin {
  ...
  func getSampleType(sampleName: String) -> HKQuantityType? {
    switch sampleName {
    case "stepCount":
    ...
    case "basalBodyTemperature":
      return HKQuantityType.quantityType(
        forIdentifier: HKQuantityTypeIdentifier.basalBodyTemperature)!
    case "bodyTemperature":
      return HKQuantityType.quantityType(forIdentifier: HKQuantityTypeIdentifier.bodyTemperature)!
    case "bloodPressureSystolic":
      return HKQuantityType.quantityType(
        forIdentifier: HKQuantityTypeIdentifier.bloodPressureSystolic)!
    case "bloodPressureDiastolic":
      return HKQuantityType.quantityType(
        forIdentifier: HKQuantityTypeIdentifier.bloodPressureDiastolic)!
  ...
  func getTimeZoneString(
    sample: HKSample? = nil, shouldReturnDefaultTimeZoneInExceptions _: Bool = true
  ) -> String {
  ...
  func generateOutput(sampleName: String, results: [HKSample]?) -> [[String: Any]]? {
    ...
    for result in results! {
      if sampleName == "sleepAnalysis" {
        ...
        let sleepState =
          (sample.value == HKCategoryValueSleepAnalysis.inBed.rawValue) ? "InBed" : "Asleep"
        ...
        output.append([
          ...
          "workoutActivityName": returnWorkoutActivityTypeValueDictionnary(
        ...
        } else if sample.quantityType.is(
          compatibleWith: HKUnit.moleUnit(withMolarMass: HKUnitMolarMassBloodGlucose).unitDivided(
            by: HKUnit.literUnit(with: .kilo)))
        {
          unit = HKUnit.moleUnit(withMolarMass: HKUnitMolarMassBloodGlucose).unitDivided(
            by: HKUnit.literUnit(with: .kilo))
          unitName = "mmol/L"
  ...
  @objc func queryHKitSampleType(_ call: CAPPluginCall) {
    ...
    let query = HKSampleQuery(
      sampleType: sampleType, predicate: predicate, limit: limit, sortDescriptors: nil
    ) {
      _, results, _ in
      guard
        let output: [[String: Any]] = self.generateOutput(sampleName: _sampleName, results: results)
      else {
  ...
  @objc func multipleQueryHKitSampleType(_ call: CAPPluginCall) {
    ...
    for _sampleName in _sampleNames {
      ...
      queryHKitSampleTypeSpecial(
        sampleName: _sampleName, startDate: _startDate, endDate: _endDate, limit: limit
      ) { result in
        switch result {
  ...
  func queryHKitSampleTypeSpecial(
    sampleName: String, startDate: Date, endDate: Date, limit: Int,
    completion: @escaping (Result<[String: Any], Error>) -> Void
  ) {
    let predicate = HKQuery.predicateForSamples(
      withStart: startDate, end: endDate, options: HKQueryOptions.strictStartDate)
    ...
    let query = HKSampleQuery(
      ...
      _, results, _ in
      guard
        let output: [[String: Any]] = self.generateOutput(sampleName: sampleName, results: results)
      else {
        return completion(.failure(HKSampleError.sampleTypeFailed))
      }
      completion(
  ...
  @objc func queryHKitSampleTypeStatisticsCollection(_ call: CAPPluginCall) async {
    ...
    guard let sampleType: HKQuantityType = getSampleType(sampleName: _sampleName) else {
      return call.reject("Error in sample name")
    }
